...
func readStoredFileWithBlocking(cfg *Config, urlPath string) (map[string]*store, error) {
	blockerFile.Lock()
	defer blockerFile.Unlock()

	return readStoredFile(cfg, urlPath)
}

// readStoreFile reads zip file and returns store object
func readStoredFile(cfg *Config, urlPath string) (map[string]*store, error) {
...
func updateStoreFile(cfg *Config, urlPath, key string, data *store) error {
	...
	out, err := readStoredFile(cfg, urlPath)
	if err != nil {
		return err
	}
...
func findKey(cfg *Config, urlPath, key string) (*store, error, bool) {
	...
	data, err := readStoredFileWithBlocking(cfg, urlPath)
	if err != nil {
...
func run(ctx context.Context, cfg *Config) error {
	...
	go func(cfg *Config, server *http.Server, listener net.Listener) {
